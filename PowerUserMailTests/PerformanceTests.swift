...
final class PerformanceTests: XCTestCase {
    ...
    private func measureOperation(
        ...
        let avgMs = totalTime / Double(iterations)
    ...
    private func printTestSummary() {
        ...
        for result in results.sorted(by: { $0.durationMs > $1.durationMs }) {
            ...
            print("\(status) [\(result.category)] \(result.name): \(String(format: "%.2f", result.durationMs))ms")
        }
    ...
    func testConversationCreation() {
        ...
        XCTAssertLessThanOrEqual(duration, targetMs, "Conversation creation should be under \(targetMs)ms")
    }
    ...
    func testCommandRegistryLookup() {
        ...
        let duration = measureOperation(name: "Command Registry Lookup", category: "Command Palette") {
    ...
    func testCommandFiltering() {
        ...
        XCTAssertLessThanOrEqual(duration, targetMs, "Command filtering should be under \(targetMs)ms")
    }
    ...
    func testFuzzySearchCommands() {
        ...
        for term in searchTerms {
            let duration = measureOperation(name: "Fuzzy Search '\(term)'", category: "Search", iterations: 20) {
                let _ = commands.filter { action in
                    action.title.lowercased().contains(term) ||
                    action.keywords.contains { $0.lowercased().contains(term) }
                }
            }
            XCTAssertLessThanOrEqual(duration, targetMs, "Fuzzy search for '\(term)' should be under \(targetMs)ms")
        }
    }
    ...
    func testFilterConversations() {
        ...
        XCTAssertLessThanOrEqual(duration, targetMs, "Filtering 100 conversations should be under \(targetMs)ms")
    }
    ...
    func testSortConversations() {
        ...
        let duration = measureOperation(name: "Sort 100 Conversations", category: "Email List") {
            let _ = conversations.sorted { ($0.messages.first?.receivedAt ?? Date()) > ($1.messages.first?.receivedAt ?? Date()) }
        }
        XCTAssertLessThanOrEqual(duration, targetMs, "Sorting 100 conversations should be under \(targetMs)ms")
    }
    ...
    func testSearchConversations() {
        ...
        XCTAssertLessThanOrEqual(duration, targetMs, "Searching 100 conversations should be under \(targetMs)ms")
    }
    ...
    func testEmailParsing() {
        ...
        let duration = measureOperation(name: "Parse Email Addresses", category: "Data", iterations: 100) {
            for email in emails {
    ...
    func testInitialsGeneration() {
        ...
        XCTAssertLessThanOrEqual(duration, targetMs, "Initials generation should be under \(targetMs)ms")
    }
    ...
    func testDateFormatting() {
        let dates = [
            ...
            Date().addingTimeInterval(-2592000)    // 30 days ago
        ]
        
        let formatter = RelativeDateTimeFormatter()
        formatter.unitsStyle = .abbreviated
        
        let duration = measureOperation(name: "Format Relative Dates", category: "UI", iterations: 100) {
            for date in dates {
                let _ = formatter.localizedString(for: date, relativeTo: Date())
            }
        }
        XCTAssertLessThanOrEqual(duration, targetMs, "Date formatting should be under \(targetMs)ms")
    }
...
final class LargeScalePerformanceTests: XCTestCase {
    ...
    func testFilter1000Conversations() {
        let conversations = (0..<1000).map { i in
            Conversation(
                id: "conv-\(i)",
                person: "User \(i)",
                messages: [
    ...
    func testCommandSearch100Times() {
        CommandLoader.loadAll()
        let searchTerms = ["new", "mark", "quit", "switch", "archive", "pin", "show", "all", "read", "email"]
